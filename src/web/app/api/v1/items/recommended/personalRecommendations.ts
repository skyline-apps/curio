...
export async function maybeUpdateAndGetPersonalRecommendations(
  ...
  const recommendationsFromDB = await fetchPersonalRecommendations(profileId);
  ...
  if (shouldRecompute) {
    await computeAndStorePersonalRecommendations(profileId);

    // Re-fetch recommendations after recomputing
    const freshRecommendations = await fetchPersonalRecommendations(profileId);
...
async function computeAndStorePersonalRecommendations(
  ...
  try {
    ...
    log.error(
      `Error computing personal recommendations for ${profileId}: ${error}`,
    );
    throw error;
  }
}
